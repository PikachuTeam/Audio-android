...
abstract class BaseActivity : AppCompatActivity(), BaseContract.View {
  ...
  private var mNumberOfRequestedPermissions = 0
  ...
  override fun onRequestPermissionsResult(requestCode: Int, permissions: Array<out String>, grantResults: IntArray) {
    if (grantResults.isNotEmpty() && grantResults.size == mNumberOfRequestedPermissions) {
  ...
  fun checkAndRequestPermissions(requestCode: Int, permissions: Array<String>): Boolean =
          permissions.filter { !checkPermission(it) }.let {
            if (it.isEmpty()) {
              ...
              mNumberOfRequestedPermissions = permissions.size
